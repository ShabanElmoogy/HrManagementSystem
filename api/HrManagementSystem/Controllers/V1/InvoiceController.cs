...
namespace SaudiEInvoice.Controllers
{
    ...
    {
        /// <summary>
        /// توليد QR Code للفاتورة الإلكترونية السعودية
        /// </summary>
        ...
        public IActionResult GenerateQRCode([FromBody] InvoiceRequest request)
        {
            try
            {
                ...
                if (string.IsNullOrWhiteSpace(request.SellerName))
                    return BadRequest(new { error = "اسم البائع مطلوب" });

                if (string.IsNullOrWhiteSpace(request.VatRegistrationNumber))
                    return BadRequest(new { error = "الرقم الضريبي مطلوب" });

                if (string.IsNullOrWhiteSpace(request.InvoiceTimestamp))
                    return BadRequest(new { error = "تاريخ الفاتورة مطلوب" });
        ...
        /// <summary>
        /// توليد QR Code وإرجاعه كصورة مباشرة
        /// </summary>
        ...
        /// <summary>
        /// دالة لتوليد QR Code حسب معايير ZATCA
        /// </summary>
